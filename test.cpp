...
public:
    ...
    void addUnion(Interval<T> newInt) {
        dl("for addUnion\n" << newInt.to_str());
        dl("this:");
        for(auto iv:intervals) dl(iv);dl("");
...
lin ext(lin lc) {
    ...
    // int esp = min()
    F esp(1e18);
    if(vec.fi != F(0)) esp = min(F(esp),F(2e7)/abs(vec.fi));
    if(vec.se != F(0)) esp = min(F(esp),F(2e7)/abs(vec.se));
    // assert(esp != 1e18);
    if(esp == 1e18) {
...
void solve()
{
    ...
    do{
        ...
        if(checkIfUnCommon(m2)) {
            dl("UnCommon for m2" << m2);
            // 不存在不可行域
        } else {
            dl("Common for m2" << m2);
            unvilRgs.addUnion(
                convexInt(getConvex(
                    {m0.fi,m2.se},  {m0.se,m2.fi}),
                    m2t) );
            dl("unvil:" + unvilRgs.to_str());
            unvilRgs.addIntersection(
                convexInt(getConvex(
                    {m0.fi,m2.fi},  {m0.se,m2.se}),
                    m2t) );
            dl("unvil:" + unvilRgs.to_str());
        }
        ...
        if(checkIfUnCommon(m0t)) {
            dl("UnCommon for m0t" << m0t);
            // 不存在不可行域
        } else {
            dl("Common for m0t" << m0t);
            unvilRgs2.addUnion(
                convexInt(getConvex(
                    {m0.fi,m0t.se},  {m0.se,m0t.fi}),
                    m2t) );
            dl("unvil:" + unvilRgs.to_str());
            unvilRgs2.addIntersection(
                convexInt(getConvex(
                    {m0.fi,m0t.fi},  {m0.se,m0t.se}),
                    m2t) );
            dl("unvil:" + unvilRgs.to_str());
        }
